...
async function patch(url, body, instance, headers = null, retriesLeft = 5) {
    const options = JSON.parse(JSON.stringify(instance.options));
    options.headers = { ...options.headers, ...headers }; 
    ...
    try {
        ...
        if (retriesLeft > 0) {
            log.debugDB(instance, "Fetcher.patch", `Fetcher: retrying left ${retriesLeft}.\n` + error.message, error.stack);
            // options.dispatcher.close();
            await delay(getDelayTime(5 - retriesLeft));
            return get(url, instance, headers, retriesLeft - 1);
        } else {
            await log.errorDB(instance, "Fetcher.patch", error.message, error.stack);
            // options.dispatcher.close();
            throw error;
        };
    };
